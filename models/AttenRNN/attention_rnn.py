...
from models.AttenRNN.main_frame import AttentionLSTMCell
from utils.tools import str2bool


class AttenRNN(nn.Module):
    def __init__(self, configs):
        ...
        for i in range(self.num_layers):
            ...
            cell_list.append(
                AttentionLSTMCell(in_channel, self.num_hidden, configs.n_heads, self.input_resolution,
    ...
    def forward(self, frames, sampling=None):
        ...
        for i in range(self.num_layers):
            ...
            h_t.append(zeros.view(B, self.input_resolution, self.input_resolution, self.num_hidden))
            c_t.append(zeros)
        ...
        for t in range(self.configs.context_length + self.configs.target_length - 1):
            ...
            # delta_c_list[0] = F.normalize(self.adapter(delta_c), dim=1)
            # delta_m_list[0] = F.normalize(self.adapter(delta_m), dim=1)

            for i in range(1, self.num_layers):
                h_t[i], c_t[i], memory, delta_c, delta_m = self.cell_list[i](h_t[i - 1], h_t[i], c_t[i], memory)
                # delta_c_list[i] = F.normalize(self.adapter(delta_c), dim=1)
                # delta_m_list[i] = F.normalize(self.adapter(delta_m), dim=1)

            # x_gen = rearrange(h_t, 'b (h w) c -> b c h w ', h=self.input_resolution)
            # x_gen = torch.sigmoid(self.head(x_gen))

            x_gen = (self.head(h_t[self.num_layers - 1].view(-1, self.num_hidden))
                     .view(B, self.input_resolution, self.input_resolution, self.frame_channel))
        ...
        next_frames = (torch.stack(next_frames, dim=0)
                       .view(self.configs.context_length + self.configs.target_length - 1, B, self.input_resolution, self.input_resolution,
                             self.configs.n_out, self.patch_size, self.patch_size)
                       .permute(1, 0, 4, 2, 5, 3, 6).contiguous()
                       .view(B, self.configs.context_length + self.configs.target_length - 1, self.configs.n_out, H, W)
                       )
...
if __name__ == "__main__":
    ...
    arg = AttenRNN.add_model_specific_args()
    args = arg.parse_args()
    model = AttenRNN(args)

    data = torch.rand(1, 19, 1, 128, 128)
    model.eval()
