...
class GatedTransformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_ratio, input_resolution, dropout=0., attn_dropout=0., drop_path=0.1):
        super().__init__()
        self.layers = nn.ModuleList([])
        self.norm = nn.LayerNorm(dim)
        self.input_resolution = input_resolution
        self.pos_embed = nn.Parameter(
            get_sinusoid_encoding_table(input_resolution*input_resolution, dim, T=10000),
            requires_grad=False,
        ).view(1, input_resolution, input_resolution,  dim)
    ...
    def forward(self, x):
        x = rearrange(x, 'b (h w) c -> b h w c', h=self.input_resolution)
...
class AttentionLSTMCell(nn.Module):
    def __init__(self, in_channel, num_hidden, num_heads, input_resolution, depth=1, mlp_ratio=4, drop_out=0.0,
        ...
        self.Atten_m = GatedTransformer(num_hidden, depth,  num_heads, mlp_ratio, input_resolution, drop_out, attn_dropout=attn_drop_out,
                                          drop_path=drop_path,)
    ...
    def forward(self, x_t, h_t, c_t, m_t):
        ...
        x = rearrange(x_t, 'b h w c -> b (h w) c')
        h_t = rearrange(h_t, 'b h w c -> b (h w) c')
        xh = self.linear_xh(torch.cat((x, h_t), dim=-1))
        xm = self.linear_xm(torch.cat((x, m_t), dim=-1))
        ...
        h_new = rearrange(h_new, 'b (h w) c -> b h w c', h=self.input_resolution)
